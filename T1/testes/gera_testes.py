...
def main():
    ...
    random_string = "".join(
        random.choices(string.ascii_letters + string.digits, k=args.base_string_size)
    )
    ...
    with open(f"{args.diretorio}/{args.base_string_size}_A.in", "w") as f:
        f.write(f"{string_a}\n")
        print(f"Teste 1 A gerado com sucesso!")

    with open(f"{args.diretorio}/{args.base_string_size}_B.in", "w") as f:
        f.write(f"{string_b}\n")
        print(f"Teste 1 B gerado com sucesso!")
    ...
    for i in range(1, args.num_testes):
        ...
        string_a += string_a
        string_b += string_b
        with open(f"{args.diretorio}/{test_size}_A.in", "w") as f:
            f.write(f"{string_a}\n")
            print(f"Teste A {i+1} gerado com sucesso!")

        with open(f"{args.diretorio}/{test_size}_B.in", "w") as f:
            f.write(f"{string_b}\n")
            print(f"Teste B {i+1} gerado com sucesso!")
